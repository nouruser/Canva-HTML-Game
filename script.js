...
let verticalVelocity;           // la vitesse verticale actuelle du ballon
let horizontalVelocity;         // la vitesse horizontale actuelle ballon

let fuel;                       // pourcentage de carburant restant
let heating;                    // Boolean: la souris est-elle enfoncée ou non?   down or up

let trees;                      // Metadata des arbres d'un tableau
let backgroundTrees;            // Metadata des arbres sur les collines en arriere-plan
...
// On va ajouter une fonction sin personnalisée qui va prendre des degrés au lieu de radians
Math.sinus = function (degree) {
  return Math.sin((degree / 180) * Math.PI);
};


// On va initialiser le layout
resetGame();


// On va réinitialiser les variables et les mises en page du jeu mais ne démarre pas le jeu(le jeu démarre à la pression d'une touche)
function resetGame() {

  // Réinitialisation de la progression du jeu
  gameStarted = false;
  heating = false;
  verticalVelocity = 5;
...
function generateTree() {
  ...
  const h = 60 + Math.random() * 80; // Height (hauteur)
...
// Si l'espave a été pressé, on va redémarrer le jeu
...
///////////
// The main game loop (la fonction principale pour le fonctionnement du jeu)
...
function animate() {
  ...
  if (heating && fuel > 0) {
    if (verticalVelocity > -8) {
      // la limite de la vitesse de montée maximale
      verticalVelocity -= velocityChangeWhileHeating;
    }
    fuel -= 0.002 * -balloonY;
  } else if (verticalVelocity < 5) {
    //la limite de la vitesse de descendante maximale
...
function drawTrees() {
  trees.forEach(({ x, h, r1, r2, r3, r4, r5, r6, r7, color }) => {
    ...
    // Crown
    // we can draw a tree using circles since a tree can be a superposition of circles
    ctx.fillStyle = color;
...
function drawBalloon() {
  ...
  // Cart
  ctx.fillStyle = "#1791B1";
  ctx.fillRect(-30, -40, 60, 10);
  ctx.fillStyle = "#1C1B4D";
  ctx.fillRect(-30, -30, 60, 30);

  // Cables
  ctx.strokeStyle = "#252131";
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(-24, -40);
  ctx.lineTo(-24, -60);
...
function drawSky() {
  ...
  gradient.addColorStop(0, "#88CDF6");
  gradient.addColorStop(1, "#BCE6FF");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, window.innerWidth, window.innerHeight);
}

// function that will draw hills 
